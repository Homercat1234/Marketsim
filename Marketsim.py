...
def run_simulation(num_buyers, num_sellers):
...
class App(tk.Tk):
    def __init__(self):
        ...
        ttk.Label(self, text="Number of Buyers:", font=('Arial', 14)).grid(column=0, row=0, padx=10, pady=5)
        ttk.Scale(self, from_=1, to=500, variable=self.num_buyers, command=self.update_buyers_label).grid(column=1, row=0, pady=5)
        ttk.Label(self, textvariable=self.num_buyers_str, font=('Arial', 14)).grid(column=2, row=0, padx=10, pady=5)

        ttk.Label(self, text="Number of Sellers:", font=('Arial', 14)).grid(column=0, row=1, padx=10, pady=5)
        ttk.Scale(self, from_=1, to=500, variable=self.num_sellers, command=self.update_sellers_label).grid(column=1, row=1, pady=5)
        ttk.Label(self, textvariable=self.num_sellers_str, font=('Arial', 14)).grid(column=2, row=1, padx=10, pady=5)

        self.update_button = ttk.Button(self, text="Update", command=self.update_values)
        self.update_button.grid(column=0, row=2, columnspan=3, padx=10, pady=10)

        # Create the figure and canvas
        self.fig, self.ax = plt.subplots(figsize=(5, 4))
        self.canvas = FigureCanvasTkAgg(self.fig, master=self)
        self.canvas.get_tk_widget().grid(column=3, row=0, rowspan=3, padx=10, pady=10)
    ...
    def update_values(self):
        ...
        sellerx, sellerprice, buyerx, buyersprice = run_simulation(num_buyers, num_sellers)
